...
static unsigned int hash(bfd_vma addr)
{
...
fetch_byte(struct disassemble_info *info)
{
  ...
  if (buffer_read_memory(info->current_addr++, &octet, 1, info) != 0) {
    // fprintf(stderr, "warning: could not read 0x%lx\n", (info->current_addr - 1));
...
Instruction* get_i386_insn(struct disassemble_info *info, bfd_vma addr, Segment *segment)
{
  ...
  inst = xalloc(1, sizeof(Instruction));
...
print_i386_mnemonic(Instruction *inst)
{
  ...
  if (inst->opcode[1]) {
    template = i386_op_2[inst->opcode[0]].mnemonic;
  }
  else if (inst->opcode[0] >= 0xD8 && inst->opcode[0] <= 0xDF) {
    ...
    if (mod != 3)
      template = float_mem[(inst->opcode[0] - 0xd8) * 8 + reg];
    else {
      struct i386_op *dp = &float_reg[inst->opcode[0] - 0xd8][reg];
      if (dp->mnemonic)
	template = dp->mnemonic;
      else
	template = fgrps[dp->bytemode1][rm];
    }
  }
  else {
    if (onebyte_is_group[inst->opcode[0]]) {
      int table_index = i386_op_1[inst->opcode[0]].bytemode1;
      template = i386_group_1[table_index][(inst->modrm >> 3) & 0x07].mnemonic;
    }
    else
      template = i386_op_1[inst->opcode[0]].mnemonic;
  }


  if (template) {
    slength = strlen(template);

    for (cnt = 0; cnt < slength; ++cnt, ++template) {
...
void print_i386_insn(Instruction *inst, int type)
{
  ...
  /* print address */
  printf(" %lx:\t", (unsigned long) inst->address);
...
static BasicBlock *extract_sub_block(BasicBlock *parent, HashTable *blocks, bfd_vma addr, bfd_vma func_start, BasicBlock **block_refer)
{
  ...
  block = xalloc(1, sizeof(BasicBlock));
...
static BasicBlock* disassemble_gap(bfd_vma first, bfd_vma last, unsigned char *grid, bfd_vma func_start)
{
  ...
  prob = xalloc((last - first + 1), sizeof(double));
  chain_length = xalloc((last - first + 1), sizeof(unsigned int));
  next_inst = xalloc((last - first + 1), sizeof(unsigned int));
...
static void disassemble_i386_function(bfd_vma start_addr, bfd_vma end_addr)
{
  bfd_vma pc, target;
  Instruction *inst;
  int type;
...
/*       break; */
  ...
  if (interact) {
    ...
    int fd;
    char template[] = "/tmp/graph.XXXXXX";
	...
	for (qe = block->conflicts.first; qe != NULL; qe = qe->next) {
      ...
      sprintf(line, "dot -Tps %s > %s.ps", template, template);
      system(line);

      sprintf(line, "gv %s.ps", template);
      system(line);

      sprintf(line, "%s.ps", template);
      unlink(line);
...
static void init_disasm_info(struct disassemble_info *disasm_info, Segment *segment)
{
...
void disassemble_i386_data (bfd *abfd, int print_insn_digraphs, int interactive)
{
  ...
  for (section = abfd->sections; section != (asection *) NULL; section = section->next) {
    ...
    if (is_code) {
      ...
      seg->status = xalloc((seg->end_addr - seg->start_addr), sizeof(char));
    
      seg->insts = xalloc((seg->end_addr - seg->start_addr), sizeof(Instruction *));
  ...
  if (print_insn_digraphs) {
    ...
    for (segment = segs; segment != NULL; segment = segment->next) {
      ...
      last = NULL;
      pc = segment->start_addr;

      while (pc < segment->end_addr) {
	curr = segment->insts[pc - segment->start_addr];
	...
	if (curr != NULL && last != NULL)
	  inst_digraph_incr(&digraphs, last, curr);

	last = curr;
      }
    }
    inst_digraph_print(&digraphs);
  }
  else if (show_asm) {
...
/* 	  break; */
	...
	if (debug)
	  printf("Segment: %lx - %lx\n", (unsigned long) segment->start_addr, (unsigned long) segment->end_addr); 
	
	fstart = pop(&segment->functions);
	if (fstart != (bfd_vma) -1) {
	  if (fstart > segment->start_addr)
	    disassemble_i386_function(segment->start_addr, fstart);
	  while ((fend = pop(&segment->functions)) != (bfd_vma) -1) {
	    disassemble_i386_function(fstart, fend);
	    fstart = fend;
	  }
	  disassemble_i386_function(fstart, segment->end_addr);
	}
	else 
	  disassemble_i386_function(segment->start_addr, segment->end_addr);
      }
    }
  }
