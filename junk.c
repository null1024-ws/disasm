  ...
  while (fgets(buf, sizeof(buf), s)) {
    ...
    if (!have_global_count) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        opcodes[i] = strtoul(opchars, NULL, 16);
      }
...
float
update_junk_score(Instruction *new, Instruction *next,
  float orig, unsigned int num)
{
  float weight_1, weight_2, cnt, result, score;

  score = score_junk_insts(next, new);
  ...
  return result;
  // return (score_junk_insts(new, next) + orig) / (num + 1);
}
